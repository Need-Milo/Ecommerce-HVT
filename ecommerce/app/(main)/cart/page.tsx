...
const CartPage = () => {
  ...
  return (
    <Container>
      ...
      {/* Thêm pb-32 md:pb-0 để tránh bị che bởi fixed bottom bar trên mobile */}
      <div className='grid lg:grid-cols-3 md:gap-8 pb-32 md:pb-0'>
              ...
              cartItems.map((item) => (
                <div key={item._id} className="border-b p-2.5 last:border-b-0 flex items-center justify-between">
                    ...
                    <div className='h-full flex flex-1 flex-col justify-between py-1'>
                      <div className='flex flex-col gap-0.5 md:gap-1.5'>
                        ...
                        <p className='text-sm capitalize'>
                          Variant: <span className='font-semibold'>{item.product.type}</span>
                        </p>
                        <p className='text-sm capitalize'>
                          Status: <span className='font-semibold'>{item.product.status}</span>
                        </p>
                      </div>
                      <Trash
                        onClick={() => {
                          dispatch(removeAllFromCart({ userId, itemId: item.id }))
                            .unwrap()
                  ...
                  <div className='flex flex-col items-start justify-between h-36 md:h-44 p-0.5 md:p-1'>
                    <PriceFormatter
                      amount={(item.product.price as number) * (item?.quantity || 0)}
        ...
        <div className='md:hidden fixed bottom-0 left-0 w-full bg-white border-t shadow-lg p-4 z-50'>
          <div className='flex items-center justify-between mb-3'>
            ...
            <Button
              onClick={handleBuy}
              className='rounded-full font-semibold tracking-wide hoverEffect px-8'
              size="lg"
            >
              Buy
            </Button>
